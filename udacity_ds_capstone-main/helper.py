...
def clean_portfolio(portfolio):
    ...
    - Split channels into separata columns
    ...
    d_chann = pd.get_dummies(portfolio_clean.channels.apply(pd.Series).stack(),
                             prefix="channel").groupby(level=1).sum()
...
def plot_offer_expense_by(customers, offer):
    ...
    plt.legend(loc='best')
    plt.title('Net Expense by Income')
    plt.xlabel('Income')
    plt.grid();
    ...
    plt.grid();
    plt.legend(loc='best');
    plt.title('Net Expense by Gender',fontsize = 16);
    plt.xlabel('Gender')
    ...
    plt.legend(loc='best')
    plt.title('Avg Transaction Value by Age')
    plt.xlabel('Age')
    plt.grid();
    ...
    plt.legend(loc='best')
    plt.title('Avg Transaction Value by Income')
    plt.xlabel('Income')
    plt.grid();
